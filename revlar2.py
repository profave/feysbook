...
logo = (f''' 
       \033[1;91m
⠀⠀⠀⠀⠀⠀     ⠀    BOOSTING TOOL{c}
       「{red}TOOL BY FEYSBOOK ENGAGEMENT{c}」{r} \n''')
...
def greet_user():
    ...
    if current_time.hour < 12:
        print(f"    Good morning, {red}{user_name}!{dark_gray}")
    elif current_time.hour < 18:
        print(f"    Good afternoon, {red}{user_name}!{dark_gray}")
    else:
        print(f"    Good evening, {red}{user_name}!{dark_gray}")
...
def ip():
    ...
    print(f" {red}  「{dark_gray}State{r}」{w}:{red}THIS IS PAID TOOL")
    ...
    print(f"{red}   「{dark_gray}s-key{r}」{w}:{red}NO NEED {red}")
    linex()
